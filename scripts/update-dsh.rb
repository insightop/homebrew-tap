...
#                → 下载 → 读出 App 内嵌版本，确认与 tag 一致（防错发包）
#                → 计算 cask 所需的 sha256
...
def fetch_latest_release
  ...
  if dmgs.size > 1
    raise "release #{tag} 中有多个 .dmg 资产，无法自动判断：#{dmgs.map { |a| a["name"] }.inspect}"
  end
...
workdir = CaskUpdate.make_workdir("update-dsh", keep_env: "DSH_KEEP_TMP")
begin
  dmg_path = File.join(workdir, rel[:asset_name])
  size = CaskUpdate.download(rel[:asset_url], dmg_path)
  log "下载完成 #{size} 字节"
